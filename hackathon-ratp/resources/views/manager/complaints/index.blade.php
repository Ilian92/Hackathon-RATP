    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-4">
        ...
        <div class="flex gap-1 bg-white rounded-xl border border-gray-200 p-1 w-fit shadow-sm">
            @foreach (['pending' => 'En attente', 'rh' => 'Transmis au RH', 'closed' => 'Clôturés'] as $value => $label)
                <a href="{{ route('manager.complaints.index', array_filter(['tab' => $value, 'type' => $typeId, 'driver_id' => $driverFilter, 'severity' => $severityFilter], fn ($v) => $v !== null && $v !== '')) }}"
        ...
        <x-filter-bar
            :action="route('manager.complaints.index')"
            :tab="$tab"
            :sort="$sort"
            :direction="$direction"
        ...
        @php
            ...
            $sortUrl = fn (string $col) => route('manager.complaints.index', array_merge($currentFilters, [
                'sort'      => $col,
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-12 text-center text-gray-400 text-sm">Aucun dossier</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
